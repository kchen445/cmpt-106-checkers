...
void Piece::getJumpsKing (vector < Point > &jumps,vector<Point> &piecesTaken,const vector<vector<Piece>> &gameBoard,const Point &direction){
	bool hasJumped = false;
    bool takenBefore = false;
	int row = jumps.back().row;
	int col = jumps.back().col;
	for(int rowAddition = -1; rowAddition < 2; rowAddition += 2){
		for(int colAddition = -1; colAddition < 2; colAddition += 2){
			...
			if(withinBounds(row,col,rowAddition,colAddition)){
                for(int i = 0; i < piecesTaken.size(); i++){
                    Point enemyPos = Point(row+rowAddition,col+colAddition);
                    if(enemyPos == piecesTaken[i]){
                        takenBefore = true;
                        break;
                    }
                }
				...
				&&withinBounds(row,col,2*rowAddition,2*colAddition)){
					...
					gameBoard.at(row + (2*rowAddition)).at(col+(2*colAddition)).isEmpty){
                        ...
                        Point posOfEnemyPiece(row+rowAddition,col+colAddition);
                        piecesTaken.push_back(posOfEnemyPiece);
						Point jumpDestination(row+(2*rowAddition),col+(2*colAddition));
						jumps.push_back(jumpDestination);
						getJumpsKing(jumps,piecesTaken,gameBoard,direction);
...
vector< vector< Point > > Piece::findMoves (const vector<vector<Piece>> &gameBoard){
	...
	//piece is a king
		...
		if(canJumpKing(gameBoard)){
			...
			vector <Point> jumps;
            vector <Point> piecesTaken;
			jumps.push_back(position);
			getJumpsKing(jumps,piecesTaken,gameBoard, initialDirection);
			return allJumps;
		}else{
