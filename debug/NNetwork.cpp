...
NNetwork<In,Out>::NNetwork(std::vector<size_t> layers) {
	...
	if (layers.size() >= 1) {
		size_t nextstart = In+Out;
		size_t nextend = nextstart + layers[0];
		...
		this->numNodes = nextend;		// = In+Out + sum(layers)
		this->connsize = numNodes-Out;
		this->biases.resize(numNodes, 0);
	} else {
		this->numNodes = In+Out;
		this->connsize = In;
		this->biases.resize(numNodes, 0);
	}
...
void NNetwork<In,Out>::mutateConnection(int chance, int retries) {
	if (randChance(chance)) {
		for (int i=0; i<retries; i++) {
			...
			//get id of end node
			size_t endid;
			if (groupstart+groupdelta < connsize) {		//groupdelta is in conns
				auto itend = itstart;			//get end node
				for (size_t i=0; i<groupdelta; i++) ++itend;
				endid = (*itend)[0].startid;	//get end node id
				if (endid < In) continue;		//check that end node is not an input node
			} else {									//groupdelta is not in conns -> output node
				endid = (groupstart+groupdelta) - connsize + In;
			}
